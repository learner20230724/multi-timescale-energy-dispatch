...
from tkinter import BOTH, BOTTOM, END, LEFT, RAISED, RIGHT, Scrollbar, StringVar, Tk
...
def _setup_mpl_fonts():
    try:
        ...
        for _f in ("Microsoft YaHei", "SimHei", "STHeiti", "WenQuanYi Micro Hei"):
...
class ResultsViewer:
    def __init__(self, root: Tk) -> None:
        ...
        self.root.geometry("1320x770")
        self.results = None

        style = ttk.Style(root)
        style.theme_use("clam")
        style.configure("Dark.TNotebook", background="#1a2530")
        style.configure("Dark.TNotebook.Tab", background="#2c3e50", foreground="white", padding=[14, 6])
        style.configure("Dark.TNotebook.Tab.selected", background="#1abc9c", foreground="white")
        style.configure("Sec.TLabel", font=("Segoe UI", 10, "bold"), foreground="#1a1a2e")
    ...
    def _build_layout(self) -> None:
        ...
        nb.add(self.tab_plot1, text="前沿与调度分析图片 (Pareto & Dispatch)")
        nb.add(self.tab_plot2, text="日内分析图片 (Intraday)")

        self._build_main_tab()
        self._build_plot_tabs()

    def _build_controls(self, parent: Frame) -> None:
        def row(label_text, var, width=6, padx=2):
    ...
    def _build_main_tab(self) -> None:
        ...
        Label(right, text="运行日志 (Run Log)", style="Sec.TLabel").pack(anchor="w", pady=(0, 4))
        ...
        def section(title):
            Label(left, text=title, style="Sec.TLabel").grid(
                row=ri[0], column=0, columnspan=3, sticky="w", pady=(8, 4)
            )
            ri[0] += 1

        def kv(text, key):
            Label(left, text=text, anchor="w").grid(row=ri[0], column=0, columnspan=2, sticky="w", pady=1)
            val = Label(left, text="—", anchor="w", foreground="#2980b9", font=("Segoe UI", 9, "bold"))
            val.grid(row=ri[0], column=2, sticky="w", pady=1)
        ...
        tbl_frame = Frame(left)
        tbl_frame.grid(row=ri[0], column=0, columnspan=3, sticky="nsew", pady=(4, 0))
        left.grid_rowconfigure(ri[0], weight=1)

        hx = Scrollbar(tbl_frame, orient="horizontal")
        hx.pack(side=BOTTOM, fill="x")

        cols = ("idx", "cost", "carbon", "curtailment")
        self.pareto_table = ttk.Treeview(
            tbl_frame, columns=cols, show="headings", height=10,
            xscrollcommand=hx.set,
        )
        hx.configure(command=self.pareto_table.xview)
    ...
    def _build_plot_tabs(self) -> None:
        ...
        fig1 = Figure(figsize=(13, 7.5), dpi=100)
        fig1.subplots_adjust(left=0.07, right=0.98, top=0.92, bottom=0.10, wspace=0.32, hspace=0.38)
        ...
        self._add_fig_label(fig1, "图1：Pareto前沿与日前调度结果")

        fig2 = Figure(figsize=(13, 4.8), dpi=100)
        fig2.subplots_adjust(left=0.05, right=0.98, top=0.88, bottom=0.15, wspace=0.28)
        ...
        self._add_fig_label(fig2, "图2：日内调度分析结果")

    def _add_fig_label(self, fig, text) -> None:
        fig.text(0.01, 0.975, text, fontsize=11, fontweight="bold",
                 va="top", ha="left", color="#2c3e50",
                 bbox=dict(boxstyle="round,pad=0.3", facecolor="#f0f4f8", alpha=0.8))

    def _log_callback(self, gen: int, cost: float, carbon: float, curt: float, pareto_n: int) -> None:
        msg = (f"[Gen {gen:4d}] Pareto数={pareto_n:3d}  "
               f"最优成本={cost:,.0f}  最优碳排={carbon:,.0f}  最优弃电率={100*curt:.4f}%\n")
        self._log(msg)
    ...
    def _run_worker(self) -> None:
        ...
        results = run_pipeline(config, log_callback=self._log_callback)
    ...
    def _do_finish(self, results) -> None:
        self.prog.stop()
    ...
    def _update_summary(self, r: dict) -> None:
        ...
        self._log(f"最终结果：成本={m.total_cost:,.2f}  碳排={m.total_carbon:,.2f}  "
    ...
    def _update_plots(self, r: dict) -> None:
        ...
        t = np.arange(96) / 4
        ax5.plot(t, id_["P_ch_day_ahead"], label="日前充电 (DA Charge)", linewidth=1.3)
        ax5.plot(t, id_["P_ch_intraday"], label="日内充电 (ID Charge)", linewidth=1.3, linestyle="--")
        ax5.plot(t, id_["P_dis_day_ahead"], label="日前放电 (DA Discharge)", linewidth=1.3)
        ax5.plot(t, id_["P_dis_intraday"], label="日内放电 (ID Discharge)", linewidth=1.3, linestyle="--")
        ax5.set_title("储能调度 (Storage Dispatch)", fontsize=10, fontweight="bold")
        ax5.legend(fontsize=8)
        ax5.set_xlabel("小时 / Hour (24h)")
        ax5.set_ylabel("MW")
        ax5.set_xticks([0, 4, 8, 12, 16, 20, 24])
        ax5.grid(True, alpha=0.3)

        ax6.plot(t, id_["P_net_day_ahead"], label="日前基准 (DA)", linewidth=1.5)
        ax6.plot(t, id_["P_net_intraday"], label="日内校正 (ID)", linewidth=1.5, linestyle="--")
        ax6.set_title("净功率不平衡 (Net Imbalance)", fontsize=10, fontweight="bold")
        ax6.legend(fontsize=8)
        ax6.set_xlabel("小时 / Hour (24h)")
        ax6.set_ylabel("MW")
    ...
    def _on_save_figures(self) -> None:
        ...
        try:
            ...
            self._save_png(path.replace(".pdf", ".png") if path.endswith(".pdf") else path)
